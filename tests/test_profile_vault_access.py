...
def test_store_round_trips_and_clears(tmp_path: Path) -> None:
    ...
    access = ProfileVaultAccess.model_validate(
        {"agent": "tutor", "session_log": {"folder": "Tutor/Sessions", "type": "tutor-session"}}
    )
    path = write_profile_vault_access("Tutor", access, tmp_path)

    assert path == tmp_path / "profile_vault_access.json"
    assert read_profile_vault_access(tmp_path)["Tutor"] == access
    write_profile_vault_access("Tutor", None, tmp_path)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        '{"version": 2, "profiles": {}}',
        '{"version": 1, "profiles": []}',
        '{"version": 1, "profiles": {"Tutor": {"agent": "Not An Id"}}}',
    ],
)
...
def test_vault_root_grant_is_kept_and_folders_are_normalized(tmp_path: Path) -> None:
    ...
    access = ProfileVaultAccess(agent="tutor", read=(".", "Tutor/Sessions/", "Tutor/Sessions", " "), write=("/",))
    write_profile_vault_access("Tutor", access, tmp_path)

    stored = read_profile_vault_access(tmp_path)["Tutor"]

    assert stored.read == ("", "Tutor/Sessions")
    assert stored.write == ("",)
...
def test_property_templates_accept_only_placeholders_the_hook_fills() -> None:
    ...
    note = {"folder": "Tutor/Sessions", "type": "tutor-session"}
    for session_properties in ({"date": "{dat}"}, {"week_of": "{week_start}"}):
        with pytest.raises(ValidationError, match="unknown placeholders"):
            ProfileVaultAccess.model_validate(
                {"agent": "tutor", "session_log": {**note, "properties": session_properties}}
            )
